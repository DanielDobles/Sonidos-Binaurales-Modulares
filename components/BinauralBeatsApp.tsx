...
export default function BinauralBeatsApp() {
  ...
  }, [isPlaying, initAudio, carrierFreq, pulseFreq, pulseType, isIsoEnabled, isoIntensity, volume, fletcherGain, isLfoEnabled, hasStarted]);
        ...
        {!hasStarted ? (
          ...
          >
            <BorderGlow className="p-12 md:p-16 pb-14 md:pb-20" borderRadius={42} glowColor={`${baseHue} 80 60`} backgroundColor="rgba(9, 9, 11, 0.8)" animated={true} glowIntensity={1.2} colors={[`hsla(${baseHue}, 80%, 60%, 1)`, `hsla(${baseHue + 40}, 70%, 50%, 1)`, '#ffffff']}>
              <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2, duration: 1.2 }} className="flex flex-col items-center gap-12 md:gap-16">
                <div className="text-center space-y-4">
                  <h1 className="text-4xl md:text-5xl font-bold font-mono uppercase tracking-[0.3em] text-white/90">Neuro-Sync</h1>
                  <p className="text-xs font-light font-mono uppercase tracking-[0.6em] text-white/30">Engine Hemi Sync</p>
                </div>
                <button onClick={toggleSound} className="group relative flex items-center justify-center w-28 h-28 rounded-full bg-white/5 border border-white/10 hover:bg-white/10 transition-all duration-700">
                  <Play className="w-10 h-10 text-white/80 fill-white/10" />
          ...
          <div key="controls-container" className="fixed inset-0 flex items-center justify-center z-10 p-4 pointer-events-none">
            <motion.div
              ...
              className="pointer-events-auto w-full max-w-[550px] bg-zinc-950/80 backdrop-blur-2xl border border-white/10 rounded-[42px] p-6 flex flex-col gap-4 relative shadow-[0_48px_100px_rgba(0,0,0,0.8)]"
            >
              <div className="flex justify-between items-center px-2">
                <h2 className="text-lg font-bold font-mono uppercase tracking-[0.25em] text-white/90">Neuro-Sync</h2>
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-3 bg-white/[0.02] border border-white/5 px-4 py-2 rounded-2xl h-10">
                    ...
                    <input type="range" min="0" max="1" step="0.05" value={volume} onChange={(e) => setVolume(parseFloat(e.target.value))} className="w-20 accent-white/40" />
                    <span className="text-[9px] font-mono text-white/30 w-7 text-right">{Math.round(volume * 100)}%</span>
                  </div>
                </div>
              </div>

              <div className="flex bg-white/[0.02] border border-white/5 p-1 rounded-2xl relative">
                <button onClick={() => setIsAutoMode(!isAutoMode)} className={cn("flex-1 py-1.5 text-[9px] font-mono uppercase tracking-wider rounded-xl transition-all flex items-center justify-center gap-2", isAutoMode ? "bg-white/10 text-white" : "text-white/30")}>
                  <Activity className={cn("w-3 h-3", isAutoMode && "animate-pulse")} /> Stochastic Sync
                </button>
                <div className="w-px h-4 bg-white/5 self-center" />
                <button onClick={() => setIsIsoEnabled(!isIsoEnabled)} className={cn("flex-1 py-1.5 text-[9px] font-mono uppercase tracking-wider rounded-xl transition-all", isIsoEnabled ? "bg-white/10 text-white" : "text-white/30")}>
                  Isochronic Layer
                </button>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {SOLFEGGIO.map((s) => (
                  <button key={s.id} onClick={() => changeSolfeggio(s.id)} className={cn("py-4 rounded-[20px] border transition-all flex flex-col items-center", activeSolfeggio === s.id ? "border-white/20 bg-white/5 text-white" : "border-white/5 bg-white/[0.02] text-white/30")}>
                    <span className="text-xs font-mono font-medium">{s.name.split(' ')[0]}</span>
                    <span className="text-[8px] font-mono opacity-40 uppercase mt-1">{s.description.split(' ')[0]}</span>
                  </button>
                ))}
              </div>
              ...
              <div className="grid grid-cols-7 gap-1.5">
                {PRESETS.map((p) => (
                  <button key={p.id} onClick={() => changePreset(p)} className={cn("flex flex-col items-center justify-center py-3 rounded-[16px] border transition-all", activePreset === p.id ? "border-white/20 bg-white/10 text-white" : "border-white/5 bg-white/[0.01] text-white/20")}>
                    {p.icon}
              ...
              <div className="flex justify-between items-center gap-4 mt-2">
                <div className="flex-1 bg-white/[0.02] py-4 rounded-[24px] border border-white/5 flex flex-col items-center gap-1">
                  ...
                  <div className="text-lg font-mono font-light text-white/80">{carrierFreq}Hz</div>
                </div>
                <button onClick={toggleSound} className="w-20 h-20 rounded-full bg-white/5 border border-white/10 flex items-center justify-center shadow-[0_0_40px_rgba(0,0,0,0.5)]">
                ...
                <div className="flex-1 bg-white/[0.02] py-4 rounded-[24px] border border-white/5 flex flex-col items-center gap-1">
                  ...
                  <div className="text-lg font-mono font-medium text-white/80 tabular-nums">
                    <span ref={pulseTextRef}>{pulseFreq.toFixed(2)}</span>Hz
                  </div>
                </div>
              </div>
              
              {isIsoEnabled && (
                <div className="w-full flex items-center justify-between px-4 h-8">
                  <span className="text-[9px] font-mono text-white/40 uppercase tracking-widest shrink-0">Intensity</span>
                  <input type="range" min="0" max="1" step="0.01" value={isoIntensity} onChange={(e) => setIsoIntensity(parseFloat(e.target.value))} className="flex-1 mx-4 accent-white/40" />
                  <span className="text-[9px] font-mono text-white/60 w-10 text-right">{Math.round(isoIntensity * 100)}%</span>
                </div>
              )}
            </motion.div>
          </div>
